	...
	public EasyuiPageOutput listForEasyuiPage(ChargeRule chargeRule) throws Exception {
		...
		if (StringUtils.isNotBlank(chargeRule.getSort())) {
			if (chargeRule.getSort().equalsIgnoreCase("groupId")) {
				...
				sortSql.append(chargeRule.getSort()).append(" ").append(chargeRule.getOrder()).append(",group_id desc")
...
//        query.setPriority(chargeRule.getPriority() + 1);
		...
		ChargeRule old = StreamEx.of(super.listByExample(query)).unordered()
				.filter(item -> item.getPriority().compareTo(chargeRule.getPriority()) > 0).findFirst().orElse(null);
		if (old == null) {
...
//    }
	...
	private List<ChargeConditionVal> parseRuleConditionVal(ChargeRule rule, ChargeRuleVo vo) {
		...
		List<ChargeConditionVal> ruleConditionVals = conditionList.stream().map((c) -> {
			...
			ValueDataTypeEnum valueDataType = ValueDataTypeEnum.fromCode(definition.getDataType()).orElse(null);
			List<Object>valueList=StreamEx.of(CollectionUtil.emptyIfNull(c.getMatchValues())).nonNull().filter(StringUtils::isNotBlank)
					.map(v -> {
						if (valueDataType == ValueDataTypeEnum.INTEGER) {
			...
			String val = JSONObject.toJSONString(valueList);
		...
		if (ActionExpressionTypeEnum.COMPLEX.equalsToCode(ruleInfo.getActionExpressionType())) {
			...
			expression.addLazyFunction(new AbstractLazyFunction("diffMinute", 2) {
				...
				public LazyNumber lazyEval(List<LazyNumber> lazyParams) {
					...
					LocalDateTime firstArg = LocalDateTime.ofEpochSecond(lazyParams.get(0).eval().longValue(), 0,
							OffsetDateTime.now().getOffset());
					LocalDateTime secondArg = LocalDateTime.ofEpochSecond(lazyParams.get(1).eval().longValue(), 0,
							OffsetDateTime.now().getOffset());
					long minutes = Duration.between(firstArg, secondArg).toMinutes();
					// System.out.println(firstArg.format( DateTimeFormatter.ofPattern("yyyy-MM-dd
					// HH:mm:ss")));
					// System.out.println(secondArg.format( DateTimeFormatter.ofPattern("yyyy-MM-dd
					// HH:mm:ss")));
			...
			expression.with("_start", new LazyNumber() {
				LocalDateTime obj = LocalDateTime.parse(startValue, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
