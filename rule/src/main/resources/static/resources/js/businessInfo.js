 ...
 * @param _defaultValue 默认值
 */
function getChargeItem(marketId, systemCode, businessType, _viewTargetId, _defaultValue) {
    let targetId = $('#' + _viewTargetId);
    targetId.empty();
    if (typeof (marketId) != "undefined" && typeof (systemCode) != "undefined" && typeof (businessType) != "undefined" && marketId && systemCode && businessType) {
        let datas = [];
        $.ajax({
            type: 'post',
            url: getChargeItemUrl,
            data: {marketId: marketId, systemCode: systemCode, businessType: businessType},
            async: false,
            success: function (ret) {
                if (ret.success) {
                    $.each(ret.data, function (k, v) {
 ...
 * @param _defaultValue 默认值
 */
function defaultGetChargeItem(_defaultValue) {
    let marketId = $('#marketId').val();
    let systemCode = $('#systemCode').val();
    let businessType = $('#businessType').val();
    getChargeItem(marketId, systemCode, businessType, 'chargeItem', _defaultValue);
}