...
(
   ...
   match_key            varchar(20) comment '匹配Key值(即：需要验证的值)',
   match_type           int comment '条件类型(大于,小于,等于)',
...
(
   ...
   target_type          tinyint not null default 0 comment '指标类型',
   data_source_id       bigint comment '数据来源ID(用于设置规则时，通过什么方法获取数据)',
   match_column         varchar(20) comment '匹配数据源中的某列值',
